...
class CRUD:
    def __init__(self, db_path: str):
        ...
        try:
            self.db = TinyDB(
                path=db_path, indent=4, separators=(",", ": "), encoding="utf-8"
            )
            self.query = Query()
    ...
    def create_one(self, item: dict):
        """
        Creates one database item unless its _id is already present
        ...
        try:
            ...
            logger.error(
                f"Unexpected item create error for _id {item.get('_id', 'unknown')}: {e}"
            )
    ...
    def find(self, key: str, value: str) -> list[dict]:
        ...
        try:
            ...
            logger.debug(
                f"Item find OK for key='{key}', value='{value}': {len(results)} matches found"
            )
            return results
            ...
            logger.error(
                f"Item find TypeError for query key='{key}', value='{value}': {e}"
            )
        except Exception as e:
            logger.error(
                f"Unexpected item find error for query key='{key}', value='{value}': {e}"
            )

    def search(self, key: str, value: str) -> list[dict]:
        """
        Performs case-insensitive search for database items
        Example: db.search("venue", "livehouse") | db.search("weekday", "Wednesday")
        ...
        try:
            ...
            logger.debug(
                f"Item search OK for key='{key}', value='{value}': {len(results)} matches found"
            )
            return results
            ...
            logger.error(
                f"Item search Regex error query key='{key}', value='{value}': {e}"
            )
        except TypeError as e:
            logger.error(
                f"Item search TypeError for query key='{key}', value='{value}': {e}"
            )
        except Exception as e:
            logger.error(
                f"Unexpected item search error for query key='{key}', value='{value}': {e}"
            )
